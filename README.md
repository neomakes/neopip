...
| 경로 | 역할 및 책임 |
| :--- | :--- |
| `01_Planning/` | **[기획]** PRD, 사용자 스토리 등 모든 기획 문서를 Markdown 파일로 관리. |
| `02_Design_Assets/` | **[디자인]** Figma Export 결과물, 아이콘, 브랜드 가이드 등 디자인 마스터 소스 보관. |
| `03_Development/` | **[코드 베이스]** 앱 소스 코드, 백엔드 로직, 데이터 모델 정의 등 모든 실행 가능한 코드가 위치. |
|     ├── `src/models` | Firebase DB 스키마 및 앱 내 데이터 구조 정의. |
|     ├── `src/services` | Firebase DB/인증과의 모든 통신 로직을 전담. |
|     └── `src/functions` | **PIP Score 계산 등 백엔드 분석 로직** (Cloud Functions) 코드. |
| `04_Distribution/` | **[배포]** App Store/Play Store 심사 자료, 스크린샷, 릴리즈 노트 등. |
